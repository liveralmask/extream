...
(function( array ){
  array.each = function( data, callback ){
    ...
    for ( var i = 0; i < data_len; ++i ){
      if ( false === callback( data[ i ], i ) ) break;
    }
  };
})(opjs.array = opjs.array || {});

(function( kvary ){
  kvary.each = function( data, callback ){
    var keys = kvary.keys( data );
    var keys_len = keys.length;
    opjs.array.each( keys, function( key, i ){
      return callback( key, data[ key ], i );
    });
  };
  
  kvary.keys = function( data ){
    return Object.keys( data );
  };
  
  kvary.length = function( data ){
    return kvary.keys( data ).length;
  };
})(opjs.kvary = opjs.kvary || {});
...
(function( string ){
  string.format = function(){
    ...
    var head = "";
    var tail = args.shift();
    if ( ( 1 === args.length ) && opjs.is_kvary( args[ 0 ] ) ) args = args[ 0 ];
...
(function( element ){
  element.create = function( tag_name, attributes, values ){
    ...
    opjs.kvary.each( attributes, function( name, attribute ){
      element.attr( _element, name, attribute );
    });
    if ( "text" in values ){
  ...
  element.tds = function( value, attributes ){
    ...
    opjs.array.each( ( value instanceof Array ) ? value : [ value ], function( data, i ){
      ...
      if ( "attributes" in data ){
        opjs.kvary.each( data.attributes, function( name, attribute, i ){
          element.attr( td, name, opjs.string.format( "{0};{1}", element.attr( td, name ), attribute ) );
        });
      }
      tds.push( td );
    });
